...
const StudentMarks = () => {
	...
	const hanldeSubmit = () => {
		getStudentMarksByRegdNum(regdNum)
			.then((res) => {
				setRegdNum("");
				setData(res.data);
				getStudentMarks().then((res) => setOverAllStudentsData(res.data));
				if (!!semester)
					navigate(`/examination/studentmarks/${regdNum}?semester=${semester}`);
				else navigate(`/examination/studentmarks/${regdNum}`);
				setError("");
			})
			.catch((err) => {
				setRegdNum("");
	...
	const getStudentMarksForGraph = () => {
		if (!!overAllStudentsData)
			if (selectedSemester != 0) {
				let deptData = overAllStudentsData
					.filter(
						(x) =>
							x.regdNo != data.regdNo &&
							x.deptId == data.deptId &&
							x.semester == selectedSemester
					)
					.map((x) => parseInt(x.overallTotal));
				let overallData = overAllStudentsData
					.filter(
						(x) => x.regdNo != data.regdNo && x.semester == selectedSemester
					)
					.map((x) => parseInt(x.overallTotal));
				...
				let deptData = overAllStudentsData
					.filter((x) => x.regdNo != data.regdNo && x.deptId == data.deptId)
					.map((x) => parseInt(x.overallTotal));
				let overallData = overAllStudentsData
					.filter((x) => x.regdNo != data.regdNo)
					.map((x) => parseInt(x.overallTotal));
				setGraphDeptData([parseInt(data.overallTotal), ...deptData]);
				setGraphCollegeData([parseInt(data.overallTotal), ...overallData]);
			}
	...
	useEffect(() => {
		if (!!regdNo) setShowStudentData(true);
		else setShowStudentData(false);
	}, [regdNo]);
			...
			<div className="flex flex-col justify-center items-center">
				{!!showStudentData ? (
					<div style={{ width: "80vw" }}>
						<div className="flex justify-between">
							...
							<div className="font-medium">
								<select
									...
									name="SelectSemester"
									id="select"
									className="ml-auto my-2 mr-2 border-solid border-2 border-sky-300 rounded"
								...
								>
									...
									{!!data &&
										data.marks.map((y, index) => (
											<option key={index} value={y.semester}>
												Semester - {y.semester}
											</option>
										))}
								</select>
						...
						{!selectedSemester && (
							<div className="d-flex justify-around flex-col items-center my-2">
								<div className="text-lg font-medium text-amber-400">
									Grand Total :{" "}
									<span className="text-orange-400 font-semibold">
										{data.overallTotal}
						...
						<div className="d-flex justify-around flex-col my-2">
							...
							<div className="text-md font-normal text-red-600 flex flex-col">
								...
								<span>
									1. Suspected Malpractice by making pledge/giving indications
									in the answer script to the examiners.
								</span>
						...
						<div className="d-flex justify-around items-center flex-col">
							<div className="text-lg font-medium text-amber-600 underline">
								How do you compare to rest of the college{" "}
								{selectedSemester != 0 && (
							...
							<div className="my-4" style={{ border: "3px solid #0060b1" }}>
								{!!getRankDeptWise() && !!getRankCollegeWise() && (
									<Chart
										className="deptStats"
										type="bar"
										width={700}
								...
								{!!semsterWiseGraphData() && (
									<Chart
										...
										options={{
											...
											xaxis: {
												tickPlacement: "on",
												categories: semsterWiseGraphData().map(
													(x) => `Sem-${x.sem}`
												),
												offsetX: 25,
											},
					...
					<Card bg="info" border="primary" className="text-center" text="white">
						<Card.Header>
							Enter Regestration number to get the details
						</Card.Header>
						<Card.Body>
							<Form>
								<Form.Group className="mb-3" controlId="formGroupEmail">
									<Form.Control
										...
										type="text"
										placeholder="Enter Regestration Num ..."
										value={regdNum}
									/>
								</Form.Group>
								<Button
						...
						{!!error && (
							<Card.Footer style={{ background: "#d42637" }}>
								{error}
							</Card.Footer>
						)}
					</Card>
